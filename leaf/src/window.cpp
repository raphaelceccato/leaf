...
namespace leaf {
	Window::Window(const char* title, int width, int height) : win(nullptr), vbo(0), vao(0) {
	...
	bool Window::pollEvent(SDL_Event* ev) {
	...
	void Window::useVSync(bool vsync) {
	...
	void Window::draw(TexturePtr tex, int x, int y, int w, int h, ShaderPtr shader) {
		int winW, winH;
	...
	void Window::drawEx(TexturePtr tex, int x, int y, int w, int h, Rect<int> subrect, float angle, ShaderPtr shader) {
		int winW, winH;
		SDL_GetWindowSize(win, &winW, &winH);
		...
		float vertices[16] = { -w / 2.0f, -h / 2.0f,					u0, v0, //vertex (2 floats), uv (2 floats)
							   w / 2.0f, -h / 2.0f,						u1, v0,
							   w / 2.0f, h / 2.0f,						u1, v1,
							   -w / 2.0f, h / 2.0f,						u0, v1 };
		glBindBuffer(GL_ARRAY_BUFFER, vbo);
		...
		model = model * glm::translate(glm::mat4(1.0f), glm::vec3(x + w / 2.0f, y + h / 2.0f, 0));
		if (angle != 0)
